...
async function getAnimeThemes (animeString) {
	...
	return themesResponse.body.anime.map(anime => anime.animethemes.filter(theme => theme.animethemeentries[0].videos.length).map(el => ({title: `${anime.name} ${el.slug} - ${el?.song?.title}`, video: el.animethemeentries[0].videos[0].link}))).flat();
}
